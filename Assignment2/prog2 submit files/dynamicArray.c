...
* Date: 4/22/2019
* Description: This program uses a dynamically
* allocated array to implment functionality for
* a stack and bag.
**********************************/
...
/**********************************************
***********************************************
Important note: I refrenced the worksheet solutions
provide, and used them in conjuction with the code 
I wrote for the worksheets from week 2. Most of the 
influence from the solutions was to reuse other methods
(i.e. the size function) rather than just hard coding it.
I saved previous versions before looking at the solutions,
...
void swapDynArr(DynArr *v, int i, int  j)
{
	/* FIXME: You will write this function */
	assert(v != 0 && v->size != 0);
	assert(i < v->size && j < v->size && j >= 0 && i >= 0);
	TYPE temp = getDynArr(v, i);
	v->data[i] = getDynArr(v, j);
	v->data[j] = temp;
