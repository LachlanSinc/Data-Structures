...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	struct Node * searchVal = tree->root;
	int boolCheck;

	while (searchVal != NULL)
	{
		boolCheck = compare(val, searchVal->val);
		if (boolCheck == 0)
			{
				return 1;
		}
		else if (boolCheck< 0) {
			searchVal = searchVal->left;
		}
		else {
			searchVal = searchVal->right;
		}
	}
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	int compareInt = compare(val, cur->val);
