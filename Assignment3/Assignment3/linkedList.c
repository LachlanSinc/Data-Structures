...
* Email: sinclala@oregonstate.edu
...
static void init(struct LinkedList* list) {
	...
	assert(list != 0);
	//create the front sentinal for the list
	list->frontSentinel = malloc(sizeof(struct Link));
	assert(list->frontSentinel != 0);

	//create the back sentinel for the list
	list->backSentinel = malloc(sizeof(struct Link));
	assert(list->backSentinel);

	//link the sentinels then set the size
...
static void addLinkBefore(struct LinkedList* list, struct Link* link, TYPE value)
{
	...
	//create the link to 
	struct Link *insertLink = (struct Link *)malloc(sizeof(struct Link));
	assert(insertLink != 0);

	//set the value for the new link then insert it into the list
	insertLink->value = value;
	insertLink->next = link;
	insertLink->prev = link->prev;
	link->prev->next = insertLink;
	link->prev = insertLink;
...
{
	/* FIXME: You will write this function */
	//calls the addLinkBefore method with the first element as the element to add
	//the link before
	assert(deque != 0);
