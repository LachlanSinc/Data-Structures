...
static void init(struct CircularList* deque)
{
	...
	struct Link *sentinelVal = malloc(sizeof(struct Link));
	assert(sentinelVal != 0);

	deque->sentinel = sentinelVal;
	sentinelVal->next = sentinelVal;
	sentinelVal->prev = sentinelVal;
...
static void addLinkAfter(struct CircularList* deque, struct Link* link, TYPE value)
{
	...
	newLink->prev = link;
	//fix the pointers of other links
...
void circularListDestroy(struct CircularList* deque)
{
	...
	//free the sentinel then the deque
	free(deque->sentinel);
	free(deque);
}
...
void circularListPrint(struct CircularList* deque)
{
	/* FIXME: You will write this function */
	assert(deque != 0);
	//create a temp pointer used to iterate through and print the list
	struct Link *temp = deque->sentinel->next;
	if (circularListIsEmpty(deque))
	{
		printf("The list is empty.\n");
	}
	//the list is not empty, loop through all elements
	else {
...
void circularListReverse(struct CircularList* deque)
{
	...
	current = temp;
	temp = temp->next;
	//loop through all remaining elements using the same steps
